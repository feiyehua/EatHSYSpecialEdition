...
function gameTapEvent(e) {
    ...
    tar = document.getElementById(_gameBBList[_gameBBListIndex].id);
...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 49) return '狗日的';
    if (score <= 99) return '你逻辑性还蛮强的嘛';
    if (score <= 149) return '我劝你还是尽早滚蛋';
    if (score <= 199) return '你这样是走不长远的';
    return '？';
}
